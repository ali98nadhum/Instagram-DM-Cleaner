...
(function () {
    ...
    function installAppIdInterceptor() {
        const w = window;
        if (w.__imcInterceptorInstalled)
            return;
    ...
    function extractAppIdFromHeaders(headers) {
        ...
        if (typeof headers === 'object') {
            for (const key of Object.keys(headers)) {
                if (key.toLowerCase() === 'x-ig-app-id')
                    return headers[key];
            }
        }
        return null;
    ...
    function getAuth() {
        ...
        if (!csrfToken)
            return { ok: false, reason: 'Not logged in. Please log in to Instagram.' };
        ...
        if (!fbDtsg)
            return { ok: false, reason: 'Could not find fb_dtsg token. Please refresh the page.' };
        const lsd = getLsd();
        if (!lsd)
            return { ok: false, reason: 'Could not find LSD token. Please refresh the page.' };
    ...
    async function fetchThreadMessages(threadFbid, cursor, auth) {
        ...
        const pageInfo = conn.page_info || { has_previous_page: false, end_cursor: null };
    ...
    function injectPanel() {
        if (document.getElementById('imc-panel'))
            return;
    ...
    async function scanMessages() {
        ...
        try {
            ...
            log(state.stopRequested
                ...
                : `Scan complete. Total: ${state.totalCount}, Yours: ${state.myMessages.length}`, state.stopRequested ? 'warn' : undefined);
        }
        catch (err) {
    ...
    function onScanClick() { scanMessages(); }
    ...
    function handleFlowError(err, phase) {
        if (err.kind === 'session') {
            ...
            if (err.body)
                log(`Response: ${String(err.body).slice(0, 300)}`, 'error');
        }
        else {
